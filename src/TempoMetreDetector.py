...
def bpm_comb_filter(signal, accuracy: int, minBpm: int, maxBpm: int, bandsLimits, samplingFrequency, combFilterPulses,
                    plotDictionary):
    n = len(signal[0])
...
class DetectMetre:
    def detectMetre(self, signal, tempo: int, bandlimits, maxFreq, npulses):
        ...
        metres = {}
        metre, metre_dft = self.four_forth(tempo, n, maxFreq, npulses)
        metres[metre] = metre_dft
        metre, metre_dft = self.three_forth(tempo, n, maxFreq, npulses)
        metres[metre] = metre_dft
        metre, metre_dft = self.five_forth(tempo, n, maxFreq, npulses)
        metres[metre] = metre_dft
        # % Initialize max energy to zero
        maxe = 0
...
if useConvolveMethod:
    song_bpm = bpm_comb_filter_convolve(diffrected, 1, first - 5, first + 5, bandLimits, samplingFrequency,
                                        plotDictionary)
else:
    song_bpm = bpm_comb_filter(diffrected, 1, first - 5, first + 5, bandLimits, samplingFrequency, combFilterPulses,
                               plotDictionary)
draw_plot(drawSongBpmEnergyPlot, list(plotDictionary.keys()), f"Tempo: {song.name}", "BPM", "Energy",
