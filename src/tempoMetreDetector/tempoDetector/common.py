...
def get_comb_filter_signal(
    ...
    if comb_filter_signal is None:
        comb_filter_signal = np.zeros(
            int(settings.fragment_length * sampling_frequency)
        )
...
def get_comb_filter_fft(sampling_frequency: int, bpm: int):
    comb_filter_fft = comb_filter_ffts.get(bpm)
    if comb_filter_fft is None:
        comb_filter_signal = get_comb_filter_signal(sampling_frequency, bpm)
